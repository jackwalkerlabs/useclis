...
for (const tool of catalog) {
  ...
  try {
    const response = await fetch(source, { signal: AbortSignal.timeout(15_000), headers: { Accept: 'application/vnd.github+json', ...(process.env.GITHUB_TOKEN ? { Authorization: `Bearer ${process.env.GITHUB_TOKEN}` } : {}) } });
    // GitHub returns 202 while calculating statistics for a repository.
    if (response.status === 202) { console.log(`${tool.name}: statistics pending, preserving existing history`); continue; }
  ...
  } catch (error) { console.error(`${tool.name}: ${error.message}. Keeping existing history.`); process.exitCode = 1; }
}
