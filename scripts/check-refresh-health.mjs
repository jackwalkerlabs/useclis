...
import { refreshRunHealth, discoveryHealth } from './lib/refresh-monitor.mjs';
...
problems.push(...discoveryHealth(JSON.parse(await readFile(new URL('../discovery/state.json', import.meta.url)))));
...
const report = `## Directory freshness and discovery\n\n${problems.length ? problems.map(problem => `- ${problem}`).join('\n') : 'Daily refresh, provider snapshots, and CLI discovery are within the documented windows.'}\n`;
console.log(report);
