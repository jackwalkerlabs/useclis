...
test('Real refresh pipeline selects new entries and preserves all existing snapshots and images; daily refresh still covers both', async () => {
  ...
  const stores = ['repositories', 'activity', 'star-snapshots', 'homebrew', 'profiles', 'downloads', 'download-history'];
  ...
  try {
    ...
    await writeFile(join(root, 'src/data/download-mappings.json'), JSON.stringify(Object.fromEntries(catalog.map(tool => [tool.slug, { repo: tool.repo, github: { assetPattern: '^cli-linux$', evidence: `https://github.com/${tool.repo}/releases` } }]))));
    ...
    const code = `
      ...
      globalThis.fetch = async url => {
        ...
        if (url.includes('/releases?')) return Response.json([{ id: 1, html_url: 'https://github.com/' + owner + '/cli/releases/tag/v1', assets: [{ id: 1, name: 'cli-linux', download_count: 42, browser_download_url: 'https://github.com/' + owner + '/cli/releases/download/v1/cli-linux' }] }]);
    ...
    assert.equal(calls.length, 8, 'Only one repository, commit, activity, owner, formula, release, and two images fetched');
    ...
    for (const store of stores) {
      ...
      if (store === 'downloads') assert.equal(data.new.github.total, 42);
    }
