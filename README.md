...
Open http://localhost:4321. The app includes a searchable, filterable leaderboard, featured listings, browser-local bookmarks, category pages, 115 CLI pages, and repository charts. No database or runtime API keys are needed.
...
- `src/data/cli-source-list.json`: the supplied 100-row list, preserving IDs, names, GitHub URLs, descriptions, and agent/AI workflow-support labels. All 100 repositories have active catalog entries; 15 additional existing CLIs are retained. Labels are joined by repository URL and shown on detail pages. “Not marked in source list” is not a claim of missing support.
...
`design-system/` contains the useclis tokens, React components, brand guidelines, and interactive CLI examples. The app imports its color, type, spacing, radius, elevation, and motion tokens and its Button component. Fonts are self-hosted variable Inconsolata for headings and body text, and the system monospace stack for commands and metrics. See the [design system guide](design-system/readme.md) and [third-party materials](THIRD_PARTY.md) for the kit's origin and redistribution status.
...
The repository is being prepared for its first public release as `jackwalkerlabs/useclis`. Original useclis code uses the [MIT license](LICENSE); provenance and applicable redistribution terms for the supplied design bundle remain to be resolved. See [release/deployment steps](docs/RELEASING.md), [contribution guide](CONTRIBUTING.md), and [third-party materials](THIRD_PARTY.md).
...
The shared header opens a responsive submission dialog. It collects a public GitHub repository, CLI name and command, category, documentation, agent use case, and optional handle. Native dialog behavior supports keyboard dismissal and focus containment; closing preserves the form for the current page visit.

Setting the build variable `PUBLIC_SUBMISSIONS_REPO=owner/repository` enables a prefilled GitHub issue handoff; the visitor must post the issue on GitHub. Without a destination, the form explicitly offers only a Markdown draft download and does not claim to submit anything. The application defaults to `jackwalkerlabs/useclis`; set the variable to override it. The GitHub repository must exist with Issues enabled before submissions can be posted. Submissions use GitHub’s own issue composer and sign-in; useclis does not request GitHub OAuth access. Creating the issue is the visitor’s final step on GitHub.

Homepage typography is aligned to the live reference's CSS sizes and weights in `src/typography.css`, including the 48px desktop / 30px mobile headline and 14px table text. This stylesheet loads after layout and mobile rules to avoid older font-size overrides. Browser rendering remains unverified.
