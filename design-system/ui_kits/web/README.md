...
Serve the repository root with a local static HTTP server and open `design-system/ui_kits/web/index.html`. JSX loading requires HTTP rather than a `file:` URL. The previews fetch pinned React and Babel scripts from a CDN and load Google Fonts; the production app bundles React and self-hosts its fonts.
